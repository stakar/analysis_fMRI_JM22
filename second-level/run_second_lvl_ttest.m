...
meta_n =1;
for session=1:2
    session = strcat('ses-',string(session));
for meta_n=1:length(metainputs_dir)
    display(meta_n)
    for crun = 1:nrun
        tmpdir = metainputs_dir(meta_n);
        inputs{1, crun} = cellstr(fullfile(resdir,session, tmpdir)); % Factorial design specification: Directory - cfg_files
        contrast1 = sprintf('con_%04d', metainputs_con(meta_n));
        inputs{2, crun} = cellstr(strcat('E:\SPM_test\results\', included_subjects, '\stories-model\',session,'\',contrast1,'.nii,1')'); % Factorial design specification: Scans - cfg_files
