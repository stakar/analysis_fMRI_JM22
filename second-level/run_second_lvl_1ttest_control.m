...
resdir = fullfile('E:\SPM_test\results_2ndLvl/1ttest_control/'); % output location
...
for meta_n=1:numel(metainputs_name)
    for crun = 1:nrun
        ...
        inputs{1, crun} = cellstr(fullfile(resdir,'ses-1', tmpdir)); % Factorial design specification: Directory - cfg_files
...
for meta_n=1:numel(metainputs_name)
    for crun = 1:nrun
        tmpdir = metainputs_dir(meta_n);
        inputs{1, crun} = cellstr(fullfile(resdir,'ses-2', tmpdir)); % Factorial design specification: Directory - cfg_files
        contrast1 = sprintf('con_%04d', meta_n);
