...
resdir = fullfile(pwd, '../results_ppi/'); % output location
...
for crun = 1:nrun
    ...
    if ~isfolder(fullfile(resdir, sub, 'ses-2'))
        % If it doesn't exist, create the directory
        mkdir(fullfile(resdir, sub, 'ses-2'))
    end

    bold = 'space-MNI152NLin2009cAsym_desc-preproc_bold';

    inputs{1, crun} = {fullfile(resdir, sub, 'ses-2/')}; % fMRI model specification: Directory - cfg_files
